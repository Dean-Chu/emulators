...
#include "../vba/Util.h"
#include "./data/vba_background.h"
#include "./data/vba_load.h"
#include "./data/vba_skingba.h"
#include "./data/vba_skingbc.h"
...
MENUITEM MainMenuItems[] = {
	...
	{"Super GameBoy: ", (int *) &GameConf.m_SuperGB, 1, (char *) &mnuOnOff, NULL},
...
void screen_prepbackground(void) {
	...
	screen_prepback(layerbackgrey, VBA_BACKGROUND, VBA_BACKGROUND_SIZE);
}
...
void screen_waitkeyarelease(void) {
	...
	while (1) {
		...
		if (keys[SDLK_a] != SDL_PRESSED) break;
	}
}
...
void screen_showtopmenu(void) {
	...
	if (!GameConf.m_ScreenRatio) {
		if(utilIsGBAImage(gameName)) {
			screen_prepback(actualScreen, VBA_SKINGBA, VBA_SKINGBA_SIZE);
			SDL_Flip(actualScreen);
			screen_prepback(actualScreen, VBA_SKINGBA, VBA_SKINGBA_SIZE);
			SDL_Flip(actualScreen);
		}
		else {
			if (GameConf.m_SuperGB == 1) {
				screen_prepback(actualScreen, VBA_SKINGBC, VBA_SKINGBC_SIZE);
				SDL_Flip(actualScreen);
				screen_prepback(actualScreen, VBA_SKINGBC, VBA_SKINGBC_SIZE);
				SDL_Flip(actualScreen);
				gbBorderOn = 1;
...
signed int load_file(char **wildcards, char *result) {
	...
	while (return_value == 1) {
		...
		while(repeat) {
			...
			screen_prepback(layer, VBA_LOAD, VBA_LOAD_SIZE);
...
char *file_ext[] = { (char *) ".agb", (char *) ".gba", (char *) ".bin", (char *) ".elf", (char *) ".mb", (char *) ".dmg",  (char *) ".gbc", (char *) ".gb", (char *) ".cgb", (char *) ".sgb", NULL };
